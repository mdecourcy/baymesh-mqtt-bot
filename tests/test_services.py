...
# StatsService tests -----------------------------------------------------------
def test_get_last_message_stats_returns_dict(stats_service: StatsService, sample_messages):
...
def test_hourly_breakdown_returns_correct_structure(stats_service: StatsService, sample_messages):
    breakdown = stats_service.get_hourly_breakdown_today()
    assert isinstance(breakdown, list)
    assert "hour" in breakdown[0]


def test_aggregation_calculations_are_correct(stats_service: StatsService, sample_messages):
    now = datetime.utcnow()
    value = stats_service.calculate_aggregation("daily_avg", now - timedelta(days=1), now)
    assert value >= 0


# SubscriptionService tests ---------------------------------------------------
def test_subscribe_user_creates_subscription(subscription_service: SubscriptionService, sample_users):
    user = sample_users[0]
...
def test_unsubscribe_user_deactivates_all(subscription_service: SubscriptionService, sample_users):
    user = sample_users[1]
...
def test_send_message_calls_subprocess(monkeypatch):
    ...
    monkeypatch.setattr("subprocess.run", MagicMock(return_value=MagicMock(stdout="ok", stderr="", returncode=0)))
    assert service.send_message(1, "hello")


def test_send_message_returns_bool(monkeypatch):
    service = MeshtasticService(cli_path="/bin/echo")
    monkeypatch.setattr("subprocess.run", MagicMock(return_value=MagicMock(stdout="", stderr="", returncode=1)))
